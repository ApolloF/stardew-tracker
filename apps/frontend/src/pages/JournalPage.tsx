...
function Icon({ emoji, label }: { emoji: string; label?: string }) {
  return <span className="emoji-icon" role={label ? 'img' : undefined} aria-label={label} aria-hidden={label ? undefined : true}>{emoji}</span>;
}
function projectEmoji(goal: any) { return goal.project?.emoji || goal.target?.emoji || '📌'; }
...
export default function JournalPage({ session, onSessionChange }: { session: Session; onSessionChange: () => void }) {
  ...
  if (!data || !session.user) {
    if (loadError) return <div className="journal-loading"><span>🐔</span><p>The journal couldn't reach the farm right now.</p><button className="journal-primary" onClick={() => { setLoadError(false); load().catch(() => { setLoadError(true); setSave('failed'); }); }}>Try again</button></div>;
    return <div className="journal-loading"><span>🌱</span><p>Opening your farm journal…</p></div>;
  }
  ...
  return <div className={`journal-app app-season-${farm.season.toLowerCase()}`}>
    ...
    <nav className="journal-tabs" aria-label="Farm journal sections">{tabs.map(([id, icon, label]) => <button key={id} className={active === id ? 'active' : ''} onClick={() => changeTab(id)} aria-current={active === id ? 'page' : undefined}><Icon emoji={icon} /><span className="tab-label">{label}</span></button>)}</nav>
...
function Briefing({ reminders, openTab }: { reminders: FarmReminder[]; openTab: (tab: TabId) => void }) {
  return <section className="briefing" aria-label="Today and tomorrow">
    {(['today', 'tomorrow'] as const).map(timing => { const rows = reminders.filter(item => item.timing === timing); return <div className={`briefing-column ${timing}`} key={timing}><h2><Icon emoji={timing === 'today' ? '🌅' : '🌙'} /> {timing === 'today' ? 'Today' : 'Tomorrow'}</h2>{rows.length ? rows.map(item => <button key={item.id} onClick={() => item.link && openTab((new URL(item.link, location.origin).searchParams.get('tab') || 'today') as TabId)}><Icon emoji={item.emoji} /><span><strong>{item.title}</strong><small>{item.detail}</small></span></button>) : <p>Nothing time-sensitive. Enjoy the day!</p>}</div>; })}
  </section>;
}
...
function TodayTab({ farm, goals, revealLateGame, openTab, isRaining, setRaining }: { farm: FarmData['farm']; goals: any[]; revealLateGame: boolean; openTab: (tab: TabId) => void; isRaining: boolean; setRaining: (raining: boolean) => void }) {
  ...
  return <div className="journal-grid">
    <section className="journal-card full planning-hints">
      <div className="section-heading"><div><small>Longer-range planning</small><h1><Icon emoji="🌟" /> Hints</h1></div><label className="weather-toggle"><input type="checkbox" checked={isRaining} onChange={event => setRaining(event.target.checked)} /><span><Icon emoji="🌧️" /> Rainy day</span></label></div>
      <div className="hint-summary">
        <article className={daysRemaining <= 4 ? 'urgent' : ''}><Icon emoji="⏳" /><div><strong>{daysRemaining === 0 ? 'Last day of the season' : `${daysRemaining} day${daysRemaining === 1 ? '' : 's'} left in ${farm.season}`}</strong><small>{daysRemaining <= 4 ? 'Harvest seasonal crops and prepare for the change.' : `The season changes after Day 28.`}</small></div><b>{daysRemaining === 0 ? 'Today' : `${daysRemaining}d`}</b></article>
        {isRaining && <article><Icon emoji="🌧️" /><div><strong>Rainy day</strong><small>No outdoor watering needed. {rainyFish.length ? `Rain-only catches now include ${rainyFish.map(fish => fish.name).join(', ')}.` : 'No rain-only seasonal catches in the catalog today.'}</small></div></article>}
        {cropDeadlines.map(({ crop, last }) => <article className={last! - farm.day <= 2 ? 'urgent' : ''} key={crop.id}><Icon emoji={crop.emoji} /><div><strong>{last === farm.day ? `Last day to plant ${crop.name}` : `${crop.name}: ${last! - farm.day} days to plant`}</strong><small>Without growth boosts, plant by {farm.season} {last} for at least one harvest.</small></div><b>{last === farm.day ? 'Today' : `${last! - farm.day}d`}</b></article>)}
      </div>
    </section>
    <section className="journal-card"><div className="section-heading"><div><small>Next two weeks</small><h1>Coming up</h1></div><button onClick={() => openTab('calendar')}>Full calendar →</button></div><div className="compact-list">{upcoming.map(item => <article key={item.id}><Icon emoji={item.emoji} /><div><strong>{item.title}</strong><small>{item.date.season} {item.date.day} · {item.detail}</small></div></article>)}{!upcoming.length && <EmptyState emoji="🍃" text="A quiet stretch in the valley — nothing on the horizon." />}</div></section>
    <section className="journal-card"><div className="section-heading"><div><small>Your plans</small><h1>Active goals</h1></div><button onClick={() => openTab('goals')}>Manage →</button></div><div className="compact-list">{visibleGoals.map(goal => <article key={goal.id}><Icon emoji={projectEmoji(goal)} /><div><strong>{goal.title}</strong><small>{formatGoalRequirements(goal)}</small></div></article>)}{!visibleGoals.length && <EmptyState emoji="🎯" text="No active goals — add one like “Build a Silo” and its steps show up here." />}</div></section>
  </div>;
}
  ...
  const card = (fish: typeof FISH[number]) => {
    ...
    return <article className={`fish-card ${done ? 'caught' : ''}`} key={fish.id}>
      <header>
        ...
        <GameIcon src={fish.iconPath} emoji={fish.emoji} label={fish.name} />
